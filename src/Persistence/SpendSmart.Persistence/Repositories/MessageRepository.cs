...
using SpendSmart.Persistence.Indexes.Messages;
using System;
...
public sealed class MessageRepository : IMessageRepository
{
    ...
    /// <inheritdoc />
    public async Task<Maybe<Message>> GetByIdAsync(Ulid messageId,
                                                   CancellationToken cancellationToken = default) 
        => await _session.LoadAsync<Message>(messageId.ToString(), cancellationToken);

    /// <inheritdoc />
    public async Task<IReadOnlyCollection<Message>> GetUnprocessedAsync(int numberOfMessages,
                                                                        CancellationToken cancellationToken = default)
    {
