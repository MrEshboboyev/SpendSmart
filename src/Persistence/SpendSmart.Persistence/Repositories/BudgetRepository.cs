...
public sealed class BudgetRepository : IBudgetRepository
{
    ...
    /// <inheritdoc />
    public async Task<Maybe<Budget>> GetByIdAsync(Ulid budgetId,
                                                  CancellationToken cancellationToken = default) 
        => await _session.LoadAsync<Budget>(budgetId.ToString(), cancellationToken);

    /// <inheritdoc />
    public async Task<Maybe<Budget>> GetByIdWithUserAsync(Ulid budgetId,
                                                          CancellationToken cancellationToken = default) 
        => await _session
                .Include(nameof(Budget.UserId))
                .LoadAsync<Budget>(budgetId.ToString(), cancellationToken);
